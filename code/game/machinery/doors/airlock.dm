...
/obj/machinery/door/airlock/attackby(var/obj/item/C, var/mob/user)
	...
	if(!repairing && (istype(C, /obj/item/weapon/weldingtool) && !( src.operating > 0 ) && src.density))
		...
		else if(statscheck(user.str, 20, 1, user, message = "Gah, I'm not strong enough to open the door. Maybe if I try again."))//Gotta be strong to get that there door open.
			if(density)
				spawn(0)	open(1)
			else
				spawn(0)	close(1)
